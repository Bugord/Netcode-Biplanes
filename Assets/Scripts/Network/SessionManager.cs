...
namespace Network
{
    ...
    public class SessionManager<T> where T : struct, ISessionPlayerData
    {
        ...
        public IReadOnlyList<T> PlayersDataList => new List<T>(clientData.Values);
        ...
        /// </summary>
        private readonly Dictionary<string, T> clientData;

        /// <summary>
        /// Map to allow us to cheaply map from player id to player data.
        /// </summary>
        private readonly Dictionary<ulong, string> clientIDToPlayerId;
        ...
        private void ReinitializePlayersData()
        {
            foreach (var id in clientIDToPlayerId.Keys) {
                ...
                var sessionPlayerData = clientData[playerId];
